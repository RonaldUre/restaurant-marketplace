...
public class OrderFactory {
    ...
    /**
     * Crea un nuevo {@link Order} en estado CREATED usando el snapshot de precios/líneas
    ...
    /**
     * Construye el request de cobro para la pasarela. Se llama fuera de transacción.
     */
    public PaymentsPort.ChargeRequest toChargeRequest(Order order, String paymentMethod) {
        return new PaymentsPort.ChargeRequest(
                order.id() != null ? order.id().value() : 0L,
                order.tenantId(),
                order.total(),
                paymentMethod
        );
