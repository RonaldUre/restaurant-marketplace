 ...
 *  - Pre-check uniqueness for slug (fast path) and rely on DB constraint as last resort.
...
public class RegisterRestaurantService implements RegisterRestaurantUseCase {
    ...
    public RestaurantView register(RegisterRestaurantCommand command) {
        ...
        // 2) Fast-path uniqueness check for slug (domain handles format; repo checks existence)
        if (restaurantRepository.existsBySlug(command.slug())) {
            throw new SlugAlreadyInUseException(command.slug());
        }

        // 3) Build domain aggregate from command (VOs inside the factory re-validate invariants)
